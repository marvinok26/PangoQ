...
class UpdateTemplateActivityRequest extends FormRequest
{
    /**
    ...
    public function withValidator($validator): void
    {
        $validator->after(function ($validator) {
            ...
            if ($tripTemplate && $this->has(['day_number', 'start_time', 'end_time'])) {
                // Check for time conflicts with existing activities on the same day (excluding current activity)
                $conflictingActivities = $tripTemplate->activities()
                    ->where('day_number', $this->day_number)
                    ->where('id', '!=', $activity->id) // Exclude current activity from conflict check
