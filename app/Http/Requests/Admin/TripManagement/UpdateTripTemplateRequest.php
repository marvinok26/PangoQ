...
class UpdateTripTemplateRequest extends FormRequest
{
    ...
    {
        $tripTemplateId = $this->route('tripTemplate')->id;
    ...
    public function withValidator($validator): void
    {
        $validator->after(function ($validator) {
            ...
            // Check if reducing duration would orphan activities
            if ($this->duration_days < $tripTemplate->duration_days) {
                $activitiesOnRemovedDays = $tripTemplate->activities()
                    ->where('day_number', '>', $this->duration_days)
                    ->count();
                
                if ($activitiesOnRemovedDays > 0) {
                    $validator->errors()->add('duration_days', 
                        "Cannot reduce duration as there are {$activitiesOnRemovedDays} activities scheduled on days that would be removed."
                    );
                }
            }
        });
